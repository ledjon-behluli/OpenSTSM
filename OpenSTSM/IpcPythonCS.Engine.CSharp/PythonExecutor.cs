...
namespace IpcPythonCS.Engine.CSharp
{
    ...
    public class PythonExecutor
    {
        ...
        public List<string> StandartOutputErrorFilters { get; }
        ...
        public PythonExecutor(string pythonPath, string scriptPath)
        {
            ...
            StandartOutputErrorFilters = new List<string>();
        }

        public PythonExecutor() : this(DEFAULT_PYTHON_PATH, DEFAULT_SCRIPT_PATH)
        {

        }

        public PythonExecutor(string pythonPath) : this(pythonPath, DEFAULT_SCRIPT_PATH)
        {
            _pythonInterpreter = new FileInfo(pythonPath);
        ...
        public void AddStandartOutputErrorFilters(string error)
        {
            if (!StandartOutputErrorFilters.Contains(error))
            {
                StandartOutputErrorFilters.Add(error);
            }
        }
        ...
        private string _runPythonInternal(string arg)
        {
            ...
            if (stderr.Peek() != -1)
            {
                string errorMsg = stderr.ReadToEnd();
                if(!StandartOutputErrorFilters.Contains(errorMsg))
                    PythonErrorMessage(errorMsg);
            }
